   ...
   <div class="box-container">
      <?php
         ...
         if(isset($_POST['filter_search'])) {
            $search_term = strtolower($_POST['location']); // Convert search term to lowercase
            error_log("Searching for location: " . $search_term);
         }

         // Modify the query to use case-insensitive search
         if(isset($_POST['filter_search'])) {
            ...
            $select_properties = $conn->prepare("SELECT * FROM `property` WHERE 
                LOWER(address) LIKE :location 
                AND type LIKE :type 
                AND offer LIKE :offer 
                AND bhk LIKE :bhk 
                AND status LIKE :status 
                AND furnished LIKE :furnished 
                AND price BETWEEN :min AND :max 
                ORDER BY date DESC");

            $select_properties->bindValue(':location', $location, PDO::PARAM_STR);
            $select_properties->bindValue(':type', "%$type%", PDO::PARAM_STR);
            $select_properties->bindValue(':offer', "%$offer%", PDO::PARAM_STR);
            $select_properties->bindValue(':bhk', "%$bhk%", PDO::PARAM_STR);
            $select_properties->bindValue(':status', "%$status%", PDO::PARAM_STR);
            $select_properties->bindValue(':furnished', "%$furnished%", PDO::PARAM_STR);
            $select_properties->bindValue(':min', $min, PDO::PARAM_INT);
            $select_properties->bindValue(':max', $max, PDO::PARAM_INT);
            
            $select_properties->execute();
            
            error_log("Query executed. Found " . $select_properties->rowCount() . " results");
         }

         if($select_properties && $select_properties->rowCount() > 0){
            ...
            $total_images = 1; // Start with 1 for image_01
            foreach(['image_02', 'image_03', 'image_04', 'image_05'] as $img) {
                if(!empty($fetch_property[$img])) {
                    $total_images++;
                }
            }
      ...
      <form action="" method="POST">
         ...
         <div class="box">
            <div class="price"><i class="fas fa-euro-sign"></i><span><?= $fetch_property['price']; ?></span></div>
      ...
      <?php
            ...
            if(isset($_POST['filter_search'])) {
                echo '<p class="empty">No properties found matching your search criteria. Try broadening your search terms.</p>';
            } else {
                echo '<p class="empty">No properties available at the moment.</p>';
            }
         }
      ?>
   </div>
